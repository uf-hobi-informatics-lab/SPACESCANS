"""
Script for preparing 250m buffer data for county level.
...
# Load the shapefile
flcounty = gpd.read_file(shape_path + 'tl_2010_12_county10/tl_2010_12_county10.shp')
...
def calculate_intersection(buffered_df, shape_df):
    ...
    overlap = gpd.overlay(buffered_df, shape_df[['GEOID10', 'geometry']], how='intersection')
    overlap['intersection_area'] = overlap.geometry.area
    overlap['value'] = overlap['intersection_area'] / overlap['area']
    overlap['fips'] = overlap['GEOID10']
    return overlap[['value', 'zip9', 'fips']]
...
p4county250m = calculate_intersection(buffer250, flcounty)

# save to CSV
p4county250m.to_csv(output_path + 'buffer250tocounty_all')

# Display the first few rows
print(p4county250m.head())
